...
void DS1307Clock::refreshTime() {                       // 2- Cohérence des données. Si on appel en succession, exemple besoin de minute et seconde,
    m_lastReadTime = m_RTC.now();                       // le temps pourrait avoir le temp de changer. Exemple temp réel à l'appel 2:59 et résultat 2:00.
...
String DS1307Clock::getActualDateTime() {                   
    ...
    String dateString = String(this->m_lastReadTime.year()) + "-" + 
                        ...
                        formatTwoDigits(this->m_lastReadTime.day()) + " " + 
                        formatTwoDigits(this->m_lastReadTime.hour()) + ":" + 
...
String DS1307Clock::getActualDate() {
    // Format: 2025-01-18
    refreshTime();
    String dateString = formatTwoDigits(this->m_lastReadTime.year()) + "-" + 
                        formatTwoDigits(this->m_lastReadTime.month()) + "-" + 
                        formatTwoDigits(this->m_lastReadTime.day());
    return dateString;
}
...
// Au besoin:
uint16_t DS1307Clock::getYear() {
    return this->m_lastReadTime.year();
}
uint8_t DS1307Clock::getMonth() {
    return this->m_lastReadTime.month();
}
uint8_t DS1307Clock::getDay() {
    return this->m_lastReadTime.day();
}
uint8_t DS1307Clock::getHour() {
    return this->m_lastReadTime.hour();
}
uint8_t DS1307Clock::getMinute() {
    return this->m_lastReadTime.minute();
}
uint8_t DS1307Clock::getSecond() {
    return this->m_lastReadTime.second();
}
